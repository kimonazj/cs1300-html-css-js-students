...
const updatePage = async () => {
  ...
  const highSugar = fruitsArray.filter(fruit => fruit.nutritions.sugar > 15);

  // TODO: Create a new HTML element to display your data
  const newElement = document.createElement('div');
  const htmlline = highSugar.map(x => "<p>"+x.name + "'s sugar level: "+x.nutritions.sugar+"<p>");
  newElement.innerHTML = htmlline.join('');
